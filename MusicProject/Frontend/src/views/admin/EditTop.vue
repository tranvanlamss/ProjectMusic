<template>
  <div class="user-add pt-20">
    <h2 style="font-weight: bold">Edit Top</h2>
    <br>
    <label>name:</label>
...
export default {
  ...
  async created() {
    try {
      const res = await axios.get('http://localhost:8080/top/get-id/' + this.$route.params.id, {
        headers: {
  ...
  methods: {
    EditTop(){
      ...
      axios
          .put("http://localhost:8080/top/update/" + this.$route.params.id,credentials, {
            headers: {
              "Authorization" : localStorage.getItem('token_admin')
            }
          })
