      ...
      <div class="w-full h-full relative overflow-y-scroll">
        ...
        <div class="px-6 py-3">
          ...
          <div class="flex items-center">
            <img style="margin-left: 10px; margin-top: 30px; object-fit: cover; width: 260px; height: 260px" :src="`${singer.image_url}`"  class="h-auto w-full shadow mb-2" alt="cover_card">
            <h1 class="text-4xl font-semibold text-white tracking-wide hover:underline" style="padding-left: 2rem">{{ singer.name }}</h1>
          </div>
        </div>
        </div>
        <!-- cards 1 -->
          ...
          <div class="pt-2 w-full flex flex-wrap">
            <div v-for="song in songs" :key="song.id" class="p-2 relative" style="width: 12rem; margin-right: 11px">
              ...
              <div class="w-full h-auto p-5 rounded-lg shadow-md" style="background: #282828">
                <img class="h-auto w-full shadow mb-2" :src="`${song.album.album_art}`" style="width: 136px; height: 136px; object-fit: cover" alt="cover_card">
...
export default {
  ...
  async created() {
    ...
    try {
      const res2 = await axios.get('http://localhost:8080/song/get-author-id/' + this.$route.params.id, {
        headers: {
          "Authorization": localStorage.getItem('token')
        }
      });
