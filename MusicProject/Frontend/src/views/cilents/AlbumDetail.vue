        ...
        <div class="px-6 py-3">
          <div class="pb-4 pl-4" style="background: #2a1441">
            <div class="flex items-center">
              <img style="margin-left: 10px; margin-top: 30px; object-fit: cover; width: 260px; height: 260px" :src="`${singer.album_art}`"  class="h-auto w-full shadow mb-2" alt="cover_card">
          ...
          <div class="pt-2 w-full flex flex-wrap">
            <div v-for="song in songs" :key="song.id" class="p-2 relative" style="width: 12rem; margin-right: 11px">
              ...
              <div class="w-full h-auto p-5 rounded-lg shadow-md" style="background: #282828">
                ...
                <h2 class="text-xs tracking-wide pb-6" style="color:#B3B3B3;">{{ song.artist }}</h2>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  async created() {
    ...
    try {
      const res2 = await axios.get('http://localhost:8080/song/get-album-id/' + this.$route.params.id, {
        headers: {
          "Authorization": localStorage.getItem('token')
        }
      });
